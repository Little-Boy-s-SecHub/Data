...
Probe dưới đây chỉ dùng cú pháp POSIX `sh` trong context đã ghi; chưa đại diện cho mọi shell hoặc runtime. Argument injection có thể xảy ra ngay cả khi không dùng shell nếu input được truyền thành option nguy hiểm cho executable; environment variable injection xảy ra khi biến môi trường do client kiểm soát ảnh hưởng loader/runtime của subprocess. [S1] [S2]

Time-based, OOB, wildcard và newline phụ thuộc shell/OS; các probe này chỉ xác nhận đúng context đã annotation. [S1] [S2]
...
**Argument Injection qua option của executable**
...
**Environment Variable Injection vào subprocess**
