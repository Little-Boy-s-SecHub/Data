...
- Nắm luồng HTTP request/response của tình huống Server-Side Template Injection (SSTI) và cách ứng dụng xử lý input qua các trust boundary.
...
### Bước 2b: Java template/expression engines
...
- Ghi log actor/session, route hoặc operation, object/resource liên quan đến Server-Side Template Injection (SSTI), kết quả policy và correlation ID; không ghi secret hoặc toàn bộ token.
...
Với Server-Side Template Injection (SSTI), các biện pháp dưới đây hỗ trợ giảm blast radius hoặc tăng khả năng phát hiện. Rate limit, UUID khó đoán, WAF, CSP hoặc validation chung không được dùng để thay thế kiểm soát gốc.
...
- **Positive case:** với Server-Side Template Injection (SSTI), luồng hợp lệ vẫn hoạt động đúng cho actor và dữ liệu được phép.
...
- Chỉ kiểm tra status code hoặc chuỗi phản hồi của Server-Side Template Injection (SSTI) mà không xác nhận side effect và log.
