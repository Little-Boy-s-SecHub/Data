...
- Nắm luồng HTTP request/response của tình huống Denial of Service Attacks và cách ứng dụng xử lý input qua các trust boundary.
...
- Mở hàng ngàn kết nối nhưng gửi dữ liệu nhỏ giọt cực chậm để máy chủ phải mệt mỏi chờ đợi (như Slowloris hoặc R.U.D.Y. với HTTP POST body chậm).
...
1. **TCP SYN Flood, Slowloris và R.U.D.Y. (Cơ bản)**:
   ...
   - **R.U.D.Y. (R-U-Dead-Yet)**: Gửi HTTP POST với `Content-Length` lớn rồi nhỏ giọt body rất chậm, khiến worker giữ request body parser lâu hơn baseline. Kiểm thử chỉ dùng một kết nối local có timeout ngắn, không chạy diện rộng.
...
- Ghi log actor/session, route hoặc operation, object/resource liên quan đến Denial of Service Attacks, kết quả policy và correlation ID; không ghi secret hoặc toàn bộ token.
...
Với Denial of Service Attacks, các biện pháp dưới đây hỗ trợ giảm blast radius hoặc tăng khả năng phát hiện. Rate limit, UUID khó đoán, WAF, CSP hoặc validation chung không được dùng để thay thế kiểm soát gốc.
...
- **Positive case:** với Denial of Service Attacks, luồng hợp lệ vẫn hoạt động đúng cho actor và dữ liệu được phép.
...
- Chỉ kiểm tra status code hoặc chuỗi phản hồi của Denial of Service Attacks mà không xác nhận side effect và log.
