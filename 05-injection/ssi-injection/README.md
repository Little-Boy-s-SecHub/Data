...
<!-- context: Apache 2.4 mod_include parses a user-controlled SSI exec directive in .shtml -->
<!-- prerequisites: non-root disposable container; command changed to printf SSI_EXEC_LAB; one render; no network -->
<!-- encoding: UTF-8 SSI directive in HTML comment syntax; double quotes delimit an ASCII command -->
<!-- expected-result: vulnerable render returns SSI_EXEC_LAB; user-content route with SSI disabled returns the directive literally -->
...
```html
<!-- The fixture evaluates a harmless command that prints a public marker. -->
<!--#exec cmd="printf SSI_EXEC_LAB" -->
```

**Tạo marker vô hại trong disposable fixture**:

<!-- payload-id: WEB-A05-SSI-INJECTION-002 -->
<!-- context: Apache httpd 2.4 SSI fixture with exec enabled inside a disposable container -->
<!-- prerequisites: no outbound network; writable /tmp; fixture destroyed after the test -->
<!-- encoding: UTF-8 SSI directive; shell redirection remains literal inside the quoted cmd attribute -->
<!-- expected-result: /tmp/ssi-lab-marker contains SSI_INJECTION_LAB -->
<!-- risk: state-changing -->
<!-- runnable: false -->
<!-- validation: static-verified -->
<!-- sources: S1 -->
<!-- last-verified: 2026-07-17 -->
```html
<!-- The fixture writes only a local marker; it opens no network connection. -->
<!--#exec cmd="printf SSI_INJECTION_LAB >/tmp/ssi-lab-marker" -->
```
...
<!-- context: Apache 2.4 SSI echo/printenv directives in a sanitized environment fixture -->
...
<!-- expected-result: vulnerable output exposes only fixture variables; secure user-content route does not evaluate directives -->
<!-- risk: non-destructive -->
<!-- runnable: false -->
<!-- validation: static-verified -->
<!-- sources: S1 -->
