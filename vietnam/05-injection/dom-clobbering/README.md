...
- Nắm luồng HTTP request/response của tình huống DOM Clobbering và cách ứng dụng xử lý input qua các trust boundary.
...
Các probe dưới đây chỉ áp dụng khi kết quả named access được chạy lại trong Chromium version đã pin. [S2] [S4]
...
- Ghi log actor/session, route hoặc operation, object/resource liên quan đến DOM Clobbering, kết quả policy và correlation ID; không ghi secret hoặc toàn bộ token.
...
Với DOM Clobbering, các biện pháp dưới đây hỗ trợ giảm blast radius hoặc tăng khả năng phát hiện. Rate limit, UUID khó đoán, WAF, CSP hoặc validation chung không được dùng để thay thế kiểm soát gốc.
...
- **Positive case:** với DOM Clobbering, luồng hợp lệ vẫn hoạt động đúng cho actor và dữ liệu được phép.
...
- Chỉ kiểm tra status code hoặc chuỗi phản hồi của DOM Clobbering mà không xác nhận side effect và log.
