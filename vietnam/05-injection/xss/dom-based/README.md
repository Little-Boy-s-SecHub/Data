...
- Nắm luồng HTTP request/response của tình huống DOM-based XSS và cách ứng dụng xử lý input qua các trust boundary.
...
- Ghi log actor/session, route hoặc operation, object/resource liên quan đến DOM-based XSS, kết quả policy và correlation ID; không ghi secret hoặc toàn bộ token.
...
Với DOM-based XSS, các biện pháp dưới đây hỗ trợ giảm blast radius hoặc tăng khả năng phát hiện. Rate limit, UUID khó đoán, WAF, CSP hoặc validation chung không được dùng để thay thế kiểm soát gốc.
...
- **Positive case:** với DOM-based XSS, luồng hợp lệ vẫn hoạt động đúng cho actor và dữ liệu được phép.
...
- Chỉ kiểm tra status code hoặc chuỗi phản hồi của DOM-based XSS mà không xác nhận side effect và log.
