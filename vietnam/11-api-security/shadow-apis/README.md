...
- Nắm luồng HTTP request/response của tình huống Shadow APIs & Improper Inventory Management và cách ứng dụng xử lý input qua các trust boundary.
...
- Ghi log actor/session, route hoặc operation, object/resource liên quan đến Shadow APIs & Improper Inventory Management, kết quả policy và correlation ID; không ghi secret hoặc toàn bộ token.
...
Với Shadow APIs & Improper Inventory Management, các biện pháp dưới đây hỗ trợ giảm blast radius hoặc tăng khả năng phát hiện. Rate limit, UUID khó đoán, WAF, CSP hoặc validation chung không được dùng để thay thế kiểm soát gốc.
...
- **Các bước chi tiết**:
  - **Maintain API inventory**: sử dụng API Gateway làm single entry point; ví dụ inventory chỉ cho phép `host=api.victim.lab.test` và `path=/api/v2/.*` trong lab.
  - **Tự động decommission API versions cũ**: gỡ route theo ngày sunset hoặc trả 410, đồng thời cập nhật owner, OpenAPI spec và telemetry.
  - **Quét định kỳ**: đối chiếu manifest, gateway route, OpenAPI spec và log truy cập để phát hiện route đang chạy nhưng không có owner/inventory.
...
- **Positive case:** với Shadow APIs & Improper Inventory Management, luồng hợp lệ vẫn hoạt động đúng cho actor và dữ liệu được phép.
...
- Chỉ kiểm tra status code hoặc chuỗi phản hồi của Shadow APIs & Improper Inventory Management mà không xác nhận side effect và log.
...
- **Zombie APIs**: Alias thường dùng cho API cũ hoặc đã bị tuyên bố ngừng hỗ trợ nhưng vẫn còn reachable, thiếu owner hoặc thiếu policy lifecycle rõ ràng.
